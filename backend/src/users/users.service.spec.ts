        ...
        UsersService,
        {
          ...
          useValue: {
            ...
            findOneBy: jest.fn().mockImplementation((condition) => 
              condition.email === 'john@example.com' ? Promise.resolve(mockUser) : Promise.resolve(null)
            ),
  ...
  describe('create', () => {
    it('should create a new user with hashed password', async () => {
      const createUserDto = {
        ...
        prenom: 'John',
        telephone: '1234567890',
        email: 'new@example.com', // Different email to avoid conflict
        password: 'password123',
        role: UserRole.MEMBRE_EQUIPE,
      };

      // Mock findOneBy to return null for this test
    ...
    it('should throw BadRequestException if email already exists', async () => {
      const existingUserDto = {
        email: 'john@example.com', // This will match our mock
        password: 'password123',
        role: UserRole.MEMBRE_EQUIPE,
      };

      await expect(service.create(existingUserDto as any))
